...
try:
    ...
    pass
try:
    from curtemp_from_sensorDS18B20 import *
except:
    pass
try:
...
class Pre_Heat_Mode(QThread):
    def run(self):
                ...
                while (n>0):
                    ...
                    if(run_or_not_preheat==0):
                        set_temp=temp_here
                        print("break it from outside fuction")
                        break
                    time.sleep(1)
                if(n<=0):
                    print("break")
        ...
        while True:
            ...
            try:
                ...
                if(cloud==0):
                    ...
                    mode_cloud=mergedata(firebase.get('/mode_cloud',None))
                    mode_state_cloud=mergedata(firebase.get('/mode_state_cloud',None))
                    if(mode_cloud==0):
                        mode_state='off'
                    elif(mode_cloud==1):
                        mode_state='cool'
                    ...
                    precool_time=mergedata(firebase.get('/precool_time',None))
                    postdata('cloud',0)
                elif(cloud==4):
                    preheat_time=mergedata(firebase.get('/preheat_time',None))
                    postdata('cloud',0)
                elif(cloud==5):
                    ...
                    sleep_time=mergedata(firebase.get('/sleep_time',None))
                    postdata('cloud',0)
                elif(cloud==13):
...
class Widget(QtGui.QWidget, Ui_themostat):
    ...
    def on_pushButton_right_pressed(self):
        ...
        if(mode_name_number==1):
            #Widget.on_pushButton_sleep_pressed(self)
            Back1.terminate()
            set_temp=set_temp+3
            run_or_not_sleep=0
...
#******************#       
    ...
    def on_pushButton_sleep_pressed(self):
        global set_temp,stop_it,mode_name_number
    ...
    def on_pushButton_cool_pressed(self):
        global set_temp , stop_it,mode_name_number,mode_state
        global run_or_not_save, run_or_not_sleep,run_or_not_precool,run_or_not_preheat
        mode_to_main_page(self,mode_state)
        if(stop_it==0):
...
def sch1():
        global set_tem, tem_1,tem_2,tem_3,tem_4,tem_5, tem_6, today
        if(today==1):
            set_tem=tem_1[1]
        if(today==2):
            set_tem=tem_1[1]
